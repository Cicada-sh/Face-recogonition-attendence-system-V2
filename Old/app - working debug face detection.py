...
def load_face_encodings():
    global face_recognizer, face_scaler, student_labels, student_names
    conn = sqlite3.connect('attendance.db')
    cursor = conn.cursor()
...
def process_attendance():
    try:
        ...
        if len(faces) > 0:
            ...
            if face_recognizer is not None and len(student_names) > 0:
                try:
                        ...
                        if student_idx < len(student_names):
                            ...
                            if not cursor.fetchone():
                                ...
                                cursor.execute('''
                                    ...
                                    VALUES (?, ?, ?, ?)
                                ''', (student_id, today, now.time(), confidence_percentage / 100))
                                conn.commit()
